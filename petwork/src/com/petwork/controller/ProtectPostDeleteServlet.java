 ...
 */
@WebServlet("/missingPet/deleteProtectPost")
public class ProtectPostDeleteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ProtectPostDeleteServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new MissingService().deleteProtectPost(postNo);
		...
		{	
			...
			if(adminMode)
			{
				loc = "/missingPet/protectPostList?adminMode=true";
			}else {
		...
		else
		{
			if(adminMode)
			{
				msg="보호신고 삭제에 실패하였습니다.";
				loc = "/missingPet/protectPostView?no = " + postNo + "&adminMode=true";
			}else {				
				msg="보호신고 삭제에 실패하였습니다. 관리자에게 문의해주세요.";
