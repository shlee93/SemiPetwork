 ...
 */
@WebServlet("/missingPet/findPostList")
public class FindPostListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FindPostListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		if(adminMode == true) {
			...
			view = "/views/missing_pet/admin_find_post_list.jsp";
		}else {
			view = "/views/missing_pet/find_post_list.jsp";
		}
		...
		int totalFind = 0;
		int totalPage = 0;
		List<FindPost> findPostList = null;
		if(category.equals("allCategory"))
		{
			if(cityCode.equals("allCity"))
			{
				...
				{	
					...
					totalFind = new MissingService().selectFindPostCount(searchCategory, searchWord);		
					totalPage = (int)Math.ceil((double)totalFind/numPerPage);						
					findPostList = new MissingService().selectFindPostList(cPage, numPerPage, searchCategory, searchWord);
					...
					totalFind = new MissingService().selectFindPostCount();
					totalPage = (int)Math.ceil((double)totalFind/numPerPage);
					findPostList = new MissingService().selectFindPostList(cPage,numPerPage);
				...
				totalFind = new MissingService().selectCityFindPostCount(cityCode);
				totalPage = (int)Math.ceil((double)totalFind/numPerPage);
				findPostList = new MissingService().selectCityFindPostList(cPage,numPerPage,cityCode);
				
				if(adminMode)
				{
					pageBarType = "&cityCode=" + cityCode  + "&adminMode=true";

				}else pageBarType = "&cityCode=" + cityCode;
			}
		}else { //선택동물 전체지역 검색미선택
			totalFind = new MissingService().selectFindPostCount(category);
			totalPage = (int)Math.ceil((double)totalFind/numPerPage);
			findPostList = new MissingService().selectFindPostList(cPage, numPerPage, category);
			
			if(adminMode)
			{
				pageBarType = "&category=" + category + "&adminMode=true";

			}else pageBarType = "&category=" + category;
		...
		else 
		{
			...
			+"/missingPet/findPostList?cPage="+(pageNo-1)
		...
		while(!(pageNo>pageEnd||pageNo>totalPage))
		{
			...
			{
				pageBar+="<a href='"+request.getContextPath()
				+"/missingPet/findPostList?cPage="+(pageNo)
