...
function fn_cityChanged(){
	...
	location.href= "<%=request.getContextPath()%>/missingPet/protectPostList?cityCode=" + cityCode;
}

function fn_categoryChanged(){
	var category = $("#post-list-category option:selected").val();
	location.href= "<%=request.getContextPath()%>/missingPet/protectPostList?category=" + category;
}
				...
				<div class="row">
					<div class="col-md-6">
						...
						<button type="button" class="btn btn-primary ml-2"
							onclick="location.href='<%=request.getContextPath()%>/missingPet/protectPostReg'">등록하기</button>
						<%} %>
					</div>
					<div class="col-md-6">
					...
					<div class="col-md-4 mt-3 p-1">
						<div class="card protect-post-list-card">
							<div>
								...
								/missingPet/protectPostView?no=<%=p.getProtectPostNo()%>">
							...
							<div class="card-block p-2 m-1 post-list-card-text">
								<div>
									<strong>지&nbsp;&nbsp;&nbsp;역</strong> <span class="m-1"> <%=addr%>
									</span>
								</div>
								<div>
				...
				<div class='row mt-4'>
					<div class="col-md-12 post-list-bottom">
						<form class="form-inline"
							action="<%=request.getContextPath()%>/missingPet/protectPostList"
							method="POST">
							...
							<input type="text" class="form-control col-md-7 mr-3"
								placeholder="검색어를 입력하세요" name="searchWord" value="<%=searchWord%>" onkeyup="this.value=this.value.replace(/[^A-Za-z0-9]/g,'')">
