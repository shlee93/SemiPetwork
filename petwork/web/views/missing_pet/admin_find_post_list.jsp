...
function fn_cityChanged(){
	...
	location.href= "<%=request.getContextPath()%>/missingPet/findPostList?cityCode=" + cityCode + "&adminMode=true";
}

function fn_categoryChanged(){
	var category = $("#post-list-category option:selected").val();
	location.href= "<%=request.getContextPath()%>/missingPet/findPostList?category=" + category + "&adminMode=true";
	}
</script>
<style>
	.nav-side-menu {
					...
					<div class="card post-list-card">
						<div>
							<a
								...
								/missingPet/findPostView?no=<%=f.getFindPostNo()%>&adminMode=true">
						...
						<div class="card-block p-3 post-list-card-text">
							<div>
								<strong>지&nbsp;&nbsp;&nbsp;역</strong> <span><%=addr%> </span>
							</div>
							<div>
							...
							<div>
								<span class="float-right my-0 pt-2" id="post-list-card-date"><%=f.getFindPostMissingDate()%></span>
							</div>
						</div>
					</div>
				</div>
			...
			<div class='row mt-4'>
				<div class="col-md-12 post-list-bottom">
					<form class="form-inline"
						action="<%=request.getContextPath()%>/missingPet/findPostList?adminMode=true"
						method="POST">
